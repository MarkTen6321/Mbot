...
spam_dataset = filter(
    lambda a: a[0].lower() == "spam", spam_dataset
)  # If first element is spam then data is spam
spam_dataset = map(
    lambda a: re.sub(r"([^\s\w]|_)+", "", a[1]).lower(), spam_dataset
)  # Only Alpha-numeric and lowercase
...
def isSFW(string):
    ...
    for word in words:
        for badword in main_words_dataset:
            ...
            if similarity(word, badword) > 7.9:
                return False
    ...
    for phrase in main_phrases_dataset:
        ...
        if len(phrase) < len(string):
            ...
            for i in range(len(string) - len(phrase)):
                ...
                if similarity(phrase, substr) > 8.5:
                    return False
...
async def handleDiscordSpam(message):
    # Check messages
    if not isSFW(message.clean_content):
...
async def spam_handle(message):
    ...
    # If user doesnt exist create a new one
        ...
        data = {"id": author_id, "username": message.author.name, "spams": 1}
...
async def handleTimeout(user, message):
    # First add user
    db.collection("discord_spam_users").document(user["id"]).set(user)
    ...
    if user["spams"] > 10:
        if message.author.roles:
            ...
            description = "> <&{1}>, I muted <@{0}>".format(
                str(message.author.id), config["mod_role"]
            )
            embed = discord.Embed(color=color, description=description)
            await message.channel.send(embed=embed)
