...
class tag(commands.Cog):
    ...
    async def tag(self, ctx, title: str, *content):
        ...
        if ctx.invoked_subcommand is None:
            ...
            tag_doc = db.document(
                "discord_tags/{0}/tags/{1}".format(ctx.guild.id, title)
            ).get()
    ...
    async def edit(self, ctx, title: str, *content):
        ...
        if tag_title.exists:
            ...
            await ctx.send(
                'A tag with the name "{0}" does not exist, create a tag using `m!addtag [tag name] [value]'.format(
                    title
                )
            )
...
async def handleTimeout(tag, ctx):
    db.collection("discord_tags").document(str(ctx.guild.id)).collection(
        "tags"
    ).document(tag["title"]).set(tag)
